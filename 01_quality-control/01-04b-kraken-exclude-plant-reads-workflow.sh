...
find . -type d -name "LIC*human-filt" > dir-list.txt

#copy the slurm script to invididual directories
cat dir-list.txt | xargs -n 1 -I \{} echo cp kraken-exclude-plant-reads-slurm.slurm \{}/ > copy-slurm.sh
...
do
 ...
 sbatch kraken-exclude-plant-reads-slurm.slurm
 cd "$OLDPWD"
done
