...
find . -type d -name "LIC*kraken2-all-excl" > dir-list.txt

#copy the slurm script to invididual directories
cat dir-list.txt | xargs -n 1 -I \{} echo cp cds-map-reads-slurm.slurm \{}/ > copy-slurm.sh
...
do
 ...
 sbatch cds-map-reads-slurm.slurm
 cd "$OLDPWD"
done
